...
const AnimatedTaskCard: React.FC<AnimatedTaskCardProps> = ({ title, icon, description, color, animationType, animationColor, isGrid, onClick }) => {
  return (
    <button
      onClick={onClick}
      className={`relative w-full p-4 rounded-2xl flex ${isGrid ? 'flex-col items-center text-center space-y-2' : 'flex-row items-center space-x-4'} shadow-[0_4px_12px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_24px_rgba(0,0,0,0.1)] hover:scale-[1.02] transition-all duration-300 min-h-[85px] overflow-hidden ${color}`}
    >
        <div className={`absolute inset-0 opacity-40 ${animationColor || ''}`}>
            <Animation type={animationType} />
        </div>
      <span className="text-3xl z-10 flex-shrink-0 drop-shadow-sm">{icon}</span>
